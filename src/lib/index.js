...
require("events").EventEmitter.prototype._maxListeners = 100; // eslint-disable-line
...
function loadSpecs(taskDescriptions, action) {
  console.time(chalk.green("loadSpecs"));
  var specs = _.flatten(
    _.map(taskDescriptions, function(taskDescription) {
      return _.map(taskDescription.content, function(testDescription) {
  ...
  console.timeEnd(chalk.green("loadSpecs"));
...
function renderScreenshot(memo, spec) {
...
function compareScreenshots(specs, action, resolve, reject, opts) {
        ...
        md5File(canonicalScreenshotPrefix + "tmp-1.png", function(err2, sum2) {
          ...
          if (sum === sum2) {
            ...
            childProcess.exec("compare " + canonicalScreenshotPrefix + "1.png " + screenshotPrefix + "1.png -metric rmse " +
                              canonicalScreenshotPrefix + "diff-1.png", function(ignored, stdout, stderr) {
              if (stderr !== "0 (0)") {
...
var tasks = {
  ...
  screenshotTask: function(action, taskDescriptions, opts, resolve, reject) {
    function runTests() {
      var specs = loadSpecs(taskDescriptions, action);

      console.time(chalk.green("all screenshots"));
      return Promise.reduce(specs, renderScreenshot, 0).
        then(sync).
        then(compareScreenshots.bind(null, specs, action, resolve, reject, opts)).
        catch(function(err) {
